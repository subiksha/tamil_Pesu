...
# Page configuration
st.set_page_config(
    page_title="Tamil TTS + SpeechT5 Voice Converter",
    page_icon="🎙️",
    layout="wide",
...
def load_speecht5_models():
    """Load SpeechT5 models (cached)"""
    global processor, model, vocoder
    
    if processor is None:
        with st.spinner("Loading SpeechT5 models (first time only)..."):
            ...
            if torch.cuda.is_available():
                ...
                st.success("✅ Using CUDA GPU for faster processing")
...
def prepare_audio_for_speecht5(audio_path, target_sr=16000):
    """Prepare audio for SpeechT5 processing"""
    # Load audio
...
def convert_voice_speecht5(source_audio_path, target_audio_path, output_path):
    ...
    try:
        ...
        # Load and prepare source audio (Tamil TTS output)
        source_audio, sr = prepare_audio_for_speecht5(source_audio_path)
        
        # Extract speaker embedding from target voice
        with st.spinner("Extracting speaker characteristics from target voice..."):
        ...
        with st.spinner("Converting voice with SpeechT5..."):
            with torch.no_grad():
...
def simple_voice_conversion(source_audio, target_audio, output_audio):
    """
    Simple voice conversion using spectral blending as fallback
    """
    try:
        # Load audio files
        ...
        # Blend spectral characteristics
        ...
        return True, "Basic conversion completed (spectral blend)"
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🎙️ Tamil TTS + SpeechT5 Voice Converter</h1>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; font-size: 1.2rem;">Convert Tamil text to speech and clone to your voice using SpeechT5</p>', unsafe_allow_html=True)
    ...
    st.markdown("""
        <div class="info-box">
            ...
            • <b>Microsoft SpeechT5</b> - Voice conversion using AI (Hugging Face)<br>
    ...
    with st.sidebar:
        ...
        if GTTS_AVAILABLE:
            ...
            st.error("❌ gTTS - pip install gtts")
        
        if SOUNDFILE_AVAILABLE:
            st.success("✅ SoundFile")
        else:
            st.warning("⚠️ SoundFile - pip install soundfile")
        
        if LIBROSA_AVAILABLE:
            st.success("✅ Librosa")
        else:
            st.warning("⚠️ Librosa - pip install librosa")
        ...
        if SPEECHT5_AVAILABLE:
            ...
            if torch.cuda.is_available():
                ...
                st.info("ℹ️ Using CPU (slower)")
        else:
            st.error("❌ SpeechT5 - pip install transformers torch")
    ...
    with col1:
        ...
        # Update text if sample was clicked
        if 'tamil_text' in st.session_state:
            tamil_text = st.session_state.tamil_text
            del st.session_state.tamil_text
    
    with col2:
        st.markdown("### 🎭 Step 2: Upload Your Voice")
        
        target_voice = st.file_uploader(
            "Upload your voice sample (WAV/MP3)",
            type=['wav', 'mp3'],
            help="Upload 5-30 seconds of your voice for best results"
        )
        
        if target_voice:
            st.success("✅ Voice sample uploaded")
            st.audio(target_voice)
            st.caption("💡 Tip: Use 5-30 seconds of clear speech for best results")
    ...
    if st.button("🚀 Generate & Clone Voice", type="primary", use_container_width=True):
        ...
        if not SPEECHT5_AVAILABLE:
            st.error("❌ SpeechT5 not available. Install with: pip install transformers torch torchaudio")
            return
        ...
        with progress_container:
            # Step 1: Generate TTS
            st.markdown("#### Step 1: Generating Tamil TTS with gTTS...")
            ...
            with st.spinner("Generating gTTS output..."):
                tts_output = output_dir / f"tamil_tts_{timestamp}.wav"
                
                # Generate gTTS and convert to wav for SpeechT5
            ...
            st.markdown("#### Step 2: Cloning your voice with SpeechT5 AI...")
            ...
            with st.spinner("Converting voice..."):
                ...
                # Output path
                converted_output = output_dir / f"cloned_voice_{timestamp}.wav"
                
                # Run SpeechT5 conversion
                ...
                # Fallback to simple conversion if SpeechT5 fails
                if not success:
                    st.warning(f"SpeechT5 failed: {message}")
                    st.info("Trying basic voice conversion...")
                    success, message = simple_voice_conversion(
                        str(tts_output),
                        str(temp_target),
                        str(converted_output)
                    )
                ...
                if success:
                    ...
                    st.error(f"❌ Voice cloning failed: {message}")
